  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // If an image was attached, upload it and send as the first message
        if (pendingImage) {
            ...
            if (uploadRes.ok) {
              ...
              await fetch(`/api/maintenance/tickets/${newTicket.id}/messages`, {
                ...
                body: JSON.stringify({
                  ...
                  content: "📎 Image attached with report",
                  imageUrl: fileUrl,
                }),
              });
            }
          } catch {
            toast.error("Ticket submitted but image failed to upload.");
          } finally {
      ...
      {showForm && (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden animate-in slide-in-from-top-4 duration-500">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            {/* Image attachment */}
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 px-1">Attach Photo (Optional)</label>
              {pendingImage ? (
                <div className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-2xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      {(pendingImage.file.size / 1024).toFixed(0)} KB · Will be sent with your report
                    </p>
                  </div>
                  <button
                    type="button"
            ...
            <button
              ...
              className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 shadow-xl shadow-blue-500/20 active:translate-y-px transition-all disabled:bg-slate-200 flex items-center justify-center gap-2"
            >
              {loading || uploadingImage ? (
                <><Loader2 size={18} className="animate-spin" />{uploadingImage ? "Uploading image..." : "Submitting..."}</>
