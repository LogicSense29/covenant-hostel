...
export default async function TenantPaymentsPage() {
  ...
  // If tenancy is EXPIRED, they must pay for a new cycle, so outstanding remaining should be the totalDue
  const remaining = user?.status === "EXPIRED" ? totalDue : Math.max(0, totalDue - verifiedTotal);
  ...
  const isRentPaid = user?.status !== "EXPIRED" && remaining === 0 && verifiedTotal > 0;
